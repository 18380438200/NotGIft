...
def changeXmlJavaToHDVersion(path):
    if False == os.path.isdir(path):
        ...
        if 0 == cmp(".java", string.lower(fileextname)):
            buildutils.swapmatchedline("import\\s+cn\\.ahabox\\.feiliwu\\.R;\\s*\\r?\\n", "import cn.ahabox.feiliwu_help.R;\n", path)
            buildutils.swapmatchedline("import\\s+cn\\.ahabox\\.feiliwu\\.BuildConfig;\\s*\\r?\\n", "import cn.ahabox.feiliwu_help.BuildConfig;\n", path)
            buildutils.swapwholefilematchedpattern("\"content://cn\\.ahabox\\.feiliwu\"", "\"content://cn.ahabox.feiliwu_help\"", path)
            #buildutils.swapmatchedline("\\s*String\\s+FULL_PACKAGE_NAME\\s+=\\s+\"cn\\.ahabox\\.feiliwu\";\\s*\\r?\\n", "		String FULL_PACKAGE_NAME = \"cn.ahabox.feiliwu_help\";\n", path)
...
if __name__ == '__main__':
    ...
    buildutils.swapmatchedline("\\s*package\\s*=\\s*\"cn\\.ahabox\\.feiliwu\"\\s*\\r?\\n", "    package=\"cn.ahabox.feiliwu_help\"\n", os.path.join(os.getcwd(), "app/src/main/AndroidManifest.xml"))
    buildutils.swapmatchedline("\\s*android\:authorities\\s*=\\s*\"cn\\.ahabox\\.feiliwu\"\\s*\\r?\\n", "            android:authorities=\"cn.ahabox.feiliwu_help\"\n", os.path.join(os.getcwd(), "app/src/main/AndroidManifest.xml"))
    #changeVersionForHD()
    buildutils.copyfolder(os.path.join(os.path.split(os.path.realpath(sys.argv[0]))[0], "changefiles"), os.getcwd())

    buildutils.swapmatchedline("\\s*android\:name\\s*=\\s*\"cn\\.ahabox\\.feiliwu\"\\s*\\r?\\n", "            android:name=\"cn.ahabox.feiliwu_help\"\n", os.path.join(os.getcwd(), "app/src/main/AndroidManifest.xml"))
    buildutils.swapmatchedline("\\s*android\:value\\s*=\\s*\"5684bf39e0f55adc5f000fc1\"\\s/>\\r?\\n", "            android:value=\"574cfe66e0f55aeaf1001ad8\" />\n", os.path.join(os.getcwd(), "app/src/main/AndroidManifest.xml"))
    buildutils.swapmatchedline("\\s*android\:value\\s*=\\s*\"5813f3c9c56473267d732ed8a9bbcb97\"\\s>\\r?\\n", "            android:value=\"1e5ef1463f27f9aacd0daa87ea4227cf\" >\n", os.path.join(os.getcwd(), "app/src/main/AndroidManifest.xml"))
    #buildutils.swapwholefilematchedpattern("File\\s+Feedback", "File Feedback2", "src/DrawerHelper.java")
    buildutils.swapwholefilematchedpattern("cn.ahabox.feiliwu", "cn.ahabox.feiliwu_help", "app/build.gradle")
    buildutils.swapwholefilematchedpattern("@string/app_name", "@string/app_name_help", "app/src/main/AndroidManifest.xml")

    #微信支付参数替换
    buildutils.swapwholefilematchedpattern("wx3336b44d23b568ae", "wx80c95ce838ce3ff7", "app/src/main/AndroidManifest.xml")
    buildutils.swapwholefilematchedpattern("wx3336b44d23b568ae", "wx80c95ce838ce3ff7", "app/src/main/java/cn/ahabox/config/Config.java")
    #服务器地址修改
    buildutils.swapwholefilematchedpattern("http://www.feiliwu.com.cn/app_api/", "http://demo.feiliwu.com.cn/app_api/", "app/src/main/java/cn/ahabox/interfaces/Api.java")
    #buildutils.swapwholefilematchedpattern("http://www.feiliwu.com.cn", "http://demo.feiliwu.com.cn", "app/src/main/java/cn/ahabox/activity/WebViewActivity.java")
