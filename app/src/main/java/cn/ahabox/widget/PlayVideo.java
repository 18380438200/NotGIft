        ...
        class MyClickListener implements View.OnClickListener{
            ...
            private void netWorkState(){
                if(NetWorkUtils.isNetWorkAvailable(context)){
                    if(NetWorkUtils.getNetworkState(context) == NetWorkState.WIFI){
                        //第一次播放，mediaPlayer为空，创建之后再加载数据
                        mediaPlayer = new MediaPlayer();
                        new PrepareThread().start();
                        llLoading.setVisibility(View.VISIBLE);
                    }else{
                        DialogUtils dialogUtils = new DialogUtils(context);
                        dialogUtils.confirmCancelDialog("当前为非wifi状态，是否继续播放", "否", "是", new CallBackimpl() {
                            @Override
                    ...
                    Toast.makeText(context,"请先检查网络",Toast.LENGTH_SHORT).show();
                }
            }
